...
class ReminderBot:
    """A class to handle reminder functionalities and session management for a chatbot."""
    ...
    def list_user_reminders(user_id: str, sender: str) -> Dict[str, str]:
        ...
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute('SELECT id, reminder_type, remind_at, interval_days FROM reminders WHERE user_id = ? AND active = 1', (user_id,))
    ...
    # Menu texts as class properties for better organization
    MAIN_MENU_TEXT = (
    ...
    def handle(self, msg: str, sender: str) -> Dict[str, str]:
        ...
        if text == "0":
            ...
            return {"reply": self.MAIN_MENU_TEXT}
        ...
        if text == "00":
            ...
            if last_menu == "" or last_menu == "main":
                ...
                return {"reply": self.REMINDER_MENU_TEXT}
        ...
        if text.lower() == "حذف":
            result = self.delete_all_reminders(sender)
            set_session(sender, {"menu": "reminder_main", "last_menu": "main"})
            return result
        ...
        if text.lower().startswith("حذف "):
            try:
                ...
                result = self.delete_reminder(sender, reminder_id)
                set_session(sender, {"menu": "reminder_main", "last_menu": "main"})
                return result
        ...
        # Reminder main menu processing
            ...
            if text == "1":
                ...
                return self.list_user_reminders(sender, sender)
            elif text == "5":
                return self.get_user_stats(sender, sender)
            else:
                set_session(sender, {"menu": "reminder_main", "last_menu": "main"})
                return {"reply": f"❌ اختر رقمًا من 1 إلى 5 أو أرسل 'حذف'.\n\n{self.REMINDER_MENU_TEXT}"}

        # Reminder date input processing
        elif current_menu == "reminder_date":
            date_info = self.is_valid_date(text)
            if date_info:
        ...
        # Reminder time input processing
            ...
            time_info = self.is_valid_time(text) if text.lower() not in ["تخطي", "skip"] else {"hour": 0, "minute": 0}
            if time_info:
        ...
        # Reminder message input processing
            ...
            if self.save_reminder(sender, reminder_type, message, remind_at, interval_days):
        ...
        # Edit reminder time input
            ...
            time_info = self.is_valid_time(text)
            if time_info:
                new_date = session.get("new_date", "")
                formatted_time = f"{time_info['hour']:02d}:{time_info['minute']:02d}"
        ...
        # Edit reminder message input
            ...
            message = None if text.lower() in ["تخطي", "skip"] else text
            result = self.update_reminder(sender, reminder_id, remind_at=new_remind_at if new_remind_at else None, message=message)
            set_session(sender, {"menu": "reminder_main", "last_menu": "main"})
            return result
...
# Instantiate the bot class for use in the handler
reminder_bot = ReminderBot()

def handle(msg: str, sender: str) -> Dict[str, str]:
    """Global handler function to process incoming messages using the ReminderBot class."""
    return reminder_bot.handle(msg, sender)
