import os import sqlite3 from datetime import datetime, timedelta from services.session import get_session, set_session import re
...
MAIN_MENU_TEXT = ( "أهلاً بك في دليل خدمات القرين\n" "يمكنك الاستعلام عن الخدمات التالية:\n\n" "1️⃣ حكومي 🏢\n" "20- منبّه 📆" )

REMINDER_MENU_TEXT = ( "⏰ منبّه\n" "اختر نوع التذكير الذي تريده:\n\n" "2️⃣ موعد مستشفى أو مناسبة\n" "6️⃣ تنبيهاتي الحالية\n\n" "❌ لحذف جميع التنبيهات أرسل: حذف\n" "↩️ للرجوع (00) | 🏠 رئيسية (0)" )

def save_reminder(sender, type_, interval, remind_at): conn = sqlite3.connect(REMINDERS_DB) c = conn.cursor() c.execute("INSERT INTO reminders (sender, type, interval_minutes, remind_at) VALUES (?, ?, ?, ?)", (sender, type_, interval, remind_at)) conn.commit() conn.close()

def list_user_reminders(sender): conn = sqlite3.connect(REMINDERS_DB) c = conn.cursor() c.execute("SELECT type, remind_at FROM reminders WHERE sender = ? AND active = 1", (sender,)) rows = c.fetchall() conn.close()
...
def delete_all_reminders(sender): conn = sqlite3.connect(REMINDERS_DB) c = conn.cursor() c.execute("UPDATE reminders SET active = 0 WHERE sender = ?", (sender,)) conn.commit() conn.close() return {"reply": "✅ تم حذف جميع التنبيهات بنجاح.\n\n↩️ للرجوع (00) | 🏠 رئيسية (0)"}
...
if session.get("menu") == "reminder_date":
    try:
        ...
        if len(parts) == 3:
            ...
            save_reminder(sender, "موعد مستشفى أو مناسبة", None, remind_at)
            set_session(sender, {"menu": "reminder_main"})
            return {
