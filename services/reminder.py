...
import sqlite3 from datetime import datetime, timedelta from typing import Optional

REMINDERS_DB = "reminders.db" SESSIONS_DB = "sessions.db"

================= قاعدة بيانات التذكيرات =================

def init_reminders_db() -> None: conn = sqlite3.connect(REMINDERS_DB) c = conn.cursor() c.execute( """ CREATE TABLE IF NOT EXISTS reminders ( id INTEGER PRIMARY KEY AUTOINCREMENT, sender TEXT NOT NULL, type TEXT NOT NULL, value TEXT, remind_at TEXT, interval_minutes INTEGER, active INTEGER DEFAULT 1 ) """ ) conn.commit() conn.close()

================= قاعدة بيانات الجلسات =================

def init_sessions_db() -> None: conn = sqlite3.connect(SESSIONS_DB) c = conn.cursor() c.execute( """ CREATE TABLE IF NOT EXISTS sessions ( sender TEXT PRIMARY KEY, current_state TEXT ) """ ) conn.commit() conn.close()

init_reminders_db() init_sessions_db()

--------- أدوات جلسات ---------

def set_session(sender: str, state: Optional[str]) -> None: conn = sqlite3.connect(SESSIONS_DB) c = conn.cursor() if state: c.execute( "REPLACE INTO sessions (sender, current_state) VALUES (?, ?)", (sender, state), ) else: c.execute("DELETE FROM sessions WHERE sender = ?", (sender,)) conn.commit() conn.close()

def get_session(sender: str) -> Optional[str]: conn = sqlite3.connect(SESSIONS_DB) c = conn.cursor() c.execute("SELECT current_state FROM sessions WHERE sender = ?", (sender,)) row = c.fetchone() conn.close() return row[0] if row else None
