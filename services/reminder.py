...
def delete_reminder(user_id: str, reminder_id: int) -> Dict[str, str]:
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        # سجل تفاصيل التذكير قبل الحذف
        cursor.execute('SELECT reminder_type, remind_at, interval_days FROM reminders WHERE user_id = ? AND id = ?', (user_id, reminder_id))
